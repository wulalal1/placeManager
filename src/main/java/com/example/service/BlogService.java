...
import com.example.common.enums.LikesMoudleEnum;
...
public class BlogService {
    ...
    public Blog selectById(Integer id) {
        ...
        for (Blog b : userBlogList) {
            Integer fid = b.getId();
            int likesCount = likesService.selectByFidAndModule(fid, LikesMoudleEnum.BLOG.getValue());
            userLikesCount += likesCount;

            int collectCount = collectService.selectByFidAndModule(fid, LikesMoudleEnum.BLOG.getValue());
        ...
        // 查询当前博客的点赞数据
        int likesCount = likesService.selectByFidAndModule(id, LikesMoudleEnum.BLOG.getValue());
        blog.setLikesCount(likesCount);
        Likes userLikes = likesService.selectUserLikes(id, LikesMoudleEnum.BLOG.getValue());
        blog.setUserLike(userLikes != null);

        // 查询当前博客的收藏数据
        int collectCount = collectService.selectByFidAndModule(id, LikesMoudleEnum.BLOG.getValue());
        blog.setCollectCount(collectCount);
        Collect userCollect = collectService.selectUserCollect(id, LikesMoudleEnum.BLOG.getValue());
    ...
    public PageInfo<Blog> selectPage(Blog blog, Integer pageNum, Integer pageSize) {
        ...
        for (Blog b : list) {
            int likesCount = likesService.selectByFidAndModule(b.getId(), LikesMoudleEnum.BLOG.getValue());
    ...
    public Set<Blog> selectRecommend(Integer blogId) {
        ...
        if (ObjectUtil.isNotEmpty(tags)) {
            ...
            for (Object tag : tagsArr) {
                // 筛选出包含当前博客标签的其他的博客列表
                Set<Blog> collect = blogList.stream().filter(b -> b.getTags() != null
                                && blogId != null
                                && b.getTags().contains(tag.toString())
